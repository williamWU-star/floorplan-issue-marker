...
export async function rest(path: string, init: RequestInit = {}) { const session = await refreshSessionIfNeeded(); return supabaseFetch(path, init, session?.access_token); }
